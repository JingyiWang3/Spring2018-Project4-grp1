...
# Save file
# save(train.mat, file = "../output/simrank_output/movie_train_mat.Rdata")
# load("../output/simrank_output/movie_train_mat.Rdata")
...
# save(w, file = "../output/simrank_output/w.Rdata")
# load("../output/simrank_output/w1.Rdata")
...
simrank.func <- function(i = 1, c = 0.8, S, w){
  for(i in 1:5){
    print(i)
    S <- c * (t(w) %*% S %*% w)
    diag(S) <- 1
...
# save(S, file = "../output/simrank_output/S.Rdata")
# load("../output/simrank_output/S.Rdata")

# s_user is the users similarity matrix
s_user <- S[1:5055, 1:5055]
# save(s_user, file = "../output/simrank_output/s_user.Rdata")
# load("../output/simrank_output/s_user.Rdata")

############### Selecting Neighborhoods - weight_threshold ################
source("movie_neighbors_select.R")
top.neighbors <- neighbors_select(s_user, 0.0003)
top.neighbors.none <- neighbors_select(s_user, 0)

###################### Prediction ######################
source("movie_prediction.R")
